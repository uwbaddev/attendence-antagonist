...
def format_discord_message(person, events):
    ...
    fields = []
    for event in events:
        # Get practice_date in a human-readable format (e.g., "Wed, Oct 2 2024, 12:00 AM")
        raw_practice_date = event.get('practice_date', '')
        try:
            if raw_practice_date:
                dt = datetime.fromisoformat(raw_practice_date.replace('Z', '+00:00'))
                # Convert to EST
                if dt.tzinfo is None:
                    dt = pytz.UTC.localize(dt)
                dt_est = dt.astimezone(EST)
            ...
            practice_date = raw_practice_date or "N/A"
